...
dht_rx_level rx_bit(idf::GPIONum pin_number, size_t time_espected,
  ...
  while (dht_pin.get_level() == level_expected) {
    if (time_count < time_espected) {
      ...
      ESP_LOGI("DHT11", "ERROR ON READING -> TIMEOUT");
      return DHT11_RX_ERR;
    }
  }
...
Dht11::Dht11(uint32_t dht_pin) : pin(dht_pin) { dht_init(this->pin); }
...
void Dht11::init_comm() {
  try {
    ...
    ESP_LOGI("DHT11", "GPIO ERROR IN COMM INICIALIZATION");
  }
}
...
TH_SampleP Dht11::read() {
  ...
  try {
    timing_err =
        check_bus_status(this->pin, RESPONSE_TIME_US, idf::GPIOLevel::LOW);
    if (timing_err != DHT11_OK) {
      ESP_LOGI("DHT11", "ERROR ON RESPONSE TIME LOW");
      return err_case;
    }
    timing_err =
        check_bus_status(this->pin, RESPONSE_TIME_US, idf::GPIOLevel::HIGH);
    if (timing_err != DHT11_OK) {
      ESP_LOGI("DHT11", "ERROR ON RESPONSE TIME HIGH");
      return err_case;
    }
    ...
    for (size_t i = 0; i < NUMBER_OF_BITS; ++i) {
      ...
      timing_err =
          check_bus_status(this->pin, DATA_BEGIN_TIME_US, idf::GPIOLevel::HIGH);
      if (timing_err != DHT11_OK) {
        ESP_LOGI("DHT11", "ERROR ON DATA BEGIN TIME");
        return err_case;
      }
      bit = rx_bit(this->pin, MAX_HIGH_BIT_TIME_US, MAX_LOW_BIT_TIME_US,
                   idf::GPIOLevel::HIGH);
